...
import '../AddBankAccount/BankAccount.css';
...
function EditBankAccount() {
    ...
    const fetchBankAccount = async () => {
      try {
        ...
        setFormData({
          ...
          balance: formatCurrency(String(account.balance * 100)),
          description: account.description,
          chart_account_id: account.chart_account_id,
          chart_account_name: account.chart_account_name
        });
      } catch (error) {
        setError('Erro ao carregar dados da conta bancária');
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/wallet/${id}`, submitData);
      setSuccess('Conta bancária atualizada com sucesso!');
      setTimeout(() => {
